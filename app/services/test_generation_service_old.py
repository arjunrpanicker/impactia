...
from ..models.test_generation import (
    StreamlinedTestGenerationRequest, TestGenerationResponse, TestCase, TestPriority, 
...
class TestGenerationService:
    ...
    async def generate_tests_streamlined(
        self, 
        pull_request_id: str,
        ...
        """Streamlined test generation with minimal required input"""
        try:
            ...
            generated_tests = await self._generate_ai_test_cases_streamlined(
            ...
            traceability_matrix = self._create_traceability_matrix_streamlined(
                modified_files, generated_tests
            )
            
            # Generate recommendations based on streamlined input
            recommendations = self._generate_recommendations_streamlined(
    ...
    async def _generate_ai_test_cases_streamlined(
        ...
        try:
            ...
            # Use analysis_input if provided (smart summary), otherwise use full analysis
            if analysis_input:
                generated_tests = await self._generate_ai_test_cases_from_summary(analysis_input, request.test_generation_options)
            else:
    ...
    async def _generate_ai_test_cases_from_summary(self, analysis_summary: str, options) -> GeneratedTests:
        """Generate test cases using AI based on smart summary (token efficient)"""
        
        # Prepare streamlined prompt for AI
        prompt = self._build_test_generation_prompt_from_summary(analysis_summary, options)
    ...
    def _build_test_generation_prompt_from_summary(self, analysis_summary: str, options) -> str:
        """Build efficient prompt for AI test case generation using smart summary"""
    ...
    async def _generate_ai_test_cases_streamlined(
        ...
        """Generate test cases using streamlined input for maximum efficiency"""
        
        # Prepare optimized prompt for AI
        prompt = self._build_streamlined_test_generation_prompt(
    ...
    def _build_streamlined_test_generation_prompt(
        self, 
        modified_files: List[str],
        smart_impact_summary: str,
        dependency_visualization: Optional[str],
        ...
        """Build ultra-efficient prompt for AI test case generation"""
        
        prompt = f"""
        Generate comprehensive test cases based on this streamlined code change analysis:
    ...
    def _create_traceability_matrix_streamlined(
        self, 
        modified_files: List[str],
        generated_tests: GeneratedTests
    ) -> TraceabilityMatrix:
        """Create simplified traceability matrix for streamlined approach without ADO dependencies"""
    ...
    def _generate_recommendations_streamlined(
        self, 
        generated_tests: GeneratedTests, 
        modified_files: List[str],
        smart_impact_summary: str
